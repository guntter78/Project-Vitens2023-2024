...
# Functie om gegevens op te halen uit de 'sensor'-tabel
def sensor_ids():
    try:
        ...
        print(f"Fout bij het ophalen van sensor_id's uit de 'sensor'-tabel: {str(e)}")
    return None
# Functie om gegevens op te halen uit de 'malfunction'-tabel
def mal_func():
    try:
        conn = create_connection()
        if conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM malfunction")
            data = cursor.fetchall()
            cursor.close()
            conn.close()
            return data
    except Exception as e:
        print(f"Fout bij het ophalen van gegevens uit de 'malfunction'-tabel: {str(e)}")
    return None

# Je bestaande functie om gegevens op te halen uit de 'flowsensor'-tabel
def fetch_data():
    try:
        conn = create_connection()
        if conn:
        ...
        print(f"Fout bij het ophalen van gegevens uit de 'flowsensor'-tabel: {str(e)}")
    return None
...
def mal():
    sensor_data = sensor_ids()
    data2 = mal_func()
    return render_template('mal.html', data=sensor_data, data2=data2)

@app.route('/update_config', methods=['POST'])
def update_config():
    if request.method == 'POST':
        selected_sensor_id = request.form.get('sensor')
        ...
        try:
            ...
            if conn:
                ...
                # Verwijder de rijen met dezelfde sensor_id
                delete_query = """
                DELETE FROM config
                WHERE sensor_id = %s;
                """
                cursor.execute(delete_query, (selected_sensor_id,))

                new_mal_code = request.form.get('malfunction')
                ...
                VALUES (%s, %s, %s);
                """
                cursor.execute(insert_query, (selected_sensor_id, new_mal_code, new_value))
                ...
                return redirect(url_for('config_updated'))
        except Exception as e:
            print(f"Fout bij het bijwerken van de configuratie: {str(e)}")

    return redirect(url_for('mal'))
@app.route('/config_updated')
def config_updated():
    return "Configuratie is bijgewerkt!"
